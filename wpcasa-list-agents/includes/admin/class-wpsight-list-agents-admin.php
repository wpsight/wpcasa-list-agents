...
class WPSight_List_Agents_Admin {
	...
	public function profile_agent_exclude( $user ) {
	    ...
	    <table class="form-table">
	        <tr>
	            <th><label for="agent_exclude"><?php _e( 'Agent Lists', 'wpsight-list-agents' ); ?></label></th>
	            <td>
	                <input type="checkbox" value="1" name="agent_exclude" id="agent_exclude" style="margin-right:5px" <?php checked( get_the_author_meta( 'agent_exclude', $user->ID ), 1 ); ?>> <?php _e( 'Hide this user from agent lists', 'wpsight-list-agents' ); ?>
	            </td>
	        </tr>
	    </table><?php
